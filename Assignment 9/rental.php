...
<FORM METHOD="post" NAME="RES_FORM" id="RES_FORM" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
...
$rentalStreetAddressErr = $rentalCityErr = $rentalStateErr = $rentalCountryErr = "";
$returnStreetAddressErr = $returnCityErr = $returnStateErr = $returnCountryErr = "";

//Pickup/Rental Location Information Fields
$rentalLocationInformationErr = "";
$pickupLocationCode = $pickupStreetAddress = $pickupCity = $pickupState = $pickupCountry = "";

//Return Location Information Fields
$returnLocationInformationErr = "";
$returnLocationCode = $returnStreetAddress = $returnCity = $returnState = $returnCountry = "";
...
//Validation for Rental/PickUp Fields 
  if (empty($_POST["PICKUP_LOCATION_CODE"]) && empty($_POST["PICKUP_STREET_ADDRESS"]) &&
empty($_POST["PICKUP_CITY_NAME"])) {
    $rentalLocationInformationErr = "*Pickup(Rental) Location Details are required. Either the Airport Location Code Or Complete Address needs to be populated";

		if(empty($_POST["PICKUP_STREET_ADDRESS"]))
    		{ $rentalStreetAddressErr = "*Street Address is required"; }
    	else
    		{ $pickupStreetAddress = $_POST["PICKUP_STREET_ADDRESS"];}

    	if(empty($_POST["PICKUP_CITY_NAME"]))
    		{ $rentalCityErr = "*City is required"; }
    	else
    		{ $pickupCity = $_POST["PICKUP_STREET_APICKUP_CITY_NAMEDDRESS"];}
     
    	if ($_POST["PICKUP_STATE"] == "0")
    		{ $rentalStateErr = "*State is required"; }
    	else
    		{ $pickupState = $_POST["PICKUP_STATE"];}

  } elseif(empty($_POST["PICKUP_LOCATION_CODE"])) 
  {
    //$pickupLocationCode = test_input($_POST["PICKUP_LOCATION_CODE"]);
    $pickupStreetAddress = test_input($_POST["PICKUP_STREET_ADDRESS"]);
    $pickupCity = test_input($_POST["PICKUP_CITY_NAME"]);
    $pickupState = test_input($_POST["PICKUP_STATE"]);
    $pickupCountry = test_input($_POST["PICKUP_COUNTRY_CODE"]);
    // check if e-mail address is well-formed
  ...
  elseif(!empty($_POST["PICKUP_LOCATION_CODE"])) 
  {
    ...
    // check if e-mail address is well-formed
    	...
    	{
    		$pickupCity = $_POST["PICKUP_CITY_NAME"];
    	}
	...
	//Validation for Return Fields
  if (empty($_POST["RETURN_LOCATION_CODE"]) && empty($_POST["RETURN_STREET_ADDRESS"]) && empty($_POST["RETURN_CITY_NAME"])) {
    $returnLocationInformationErr = "*Return Location Details are required. Either the Airport Location Code Or Complete Address needs to be populated";

		if(empty($_POST["RETURN_STREET_ADDRESS"]))
    		{ $returnStreetAddressErr = "*Street Address is required"; }

    	if(empty($_POST["RETURN_CITY_NAME"]))
    		{ $returnCityErr = "*City is required"; }
     
    	if ($_POST["RETURN_STATE"] == "0")
    		{ $returnStateErr = "*State is required"; }

  } elseif(empty($_POST["RETURN_LOCATION_CODE"])) 
  {
    //$pickupLocationCode = test_input($_POST["PICKUP_LOCATION_CODE"]);
    $returnStreetAddress = test_input($_POST["RETURN_STREET_ADDRESS"]);
    $returnCity = test_input($_POST["RETURN_CITY_NAME"]);
    $returnState = test_input($_POST["RETURN_STATE"]);
    $returnCountry = test_input($_POST["RETURN_COUNTRY_CODE"]);
    // check if e-mail address is well-formed
  ...
  }elseif(!empty($_POST["RETURN_LOCATION_CODE"])) 
  {
    ...
    // check if e-mail address is well-formed
    	...
    	if(empty($_POST["RETURN_STREET_ADDRESS"]))
    		{ $returnStreetAddressErr = ""; }
    	else
    	{
    		$returnStreetAddress = $_POST["RETURN_STREET_ADDRESS"];
    	}
    	...
    	{
    		$returnCity = $_POST["RETURN_CITY_NAME"];
    	}
    	...
    	{
    		$returnState = $_POST["RETURN_STATE"];
    	}

  } 
    
 	if(!isset($_POST["CAR_GROUP_CODE"])){
		$carClassErr = "*Please select any one car class";
 	}
...
<fieldset><legend><h3>Rental Location Information</h3></legend>
	<span class="error" style="color:red"> <?php echo $rentalLocationInformationErr;?></span>
<TABLE WIDTH="100%" CELLSPACING="0" CELLPADDING="0" BORDER="0">
   <TR>
	<TD width="35%">&nbsp;</TD>
 ...
 <TR>
   ...
   <TR>
	<TD width="35%"><INPUT TYPE=text NAME="PICKUP_LOCATION_CODE" SIZE=3  MAXLENGTH=3 value="<?php echo $pickupLocationCode;?>">(if known)</TD>
	<td width="40%" valign=middle>&nbsp;</td>
	<TD width="25%"><INPUT TYPE=text NAME="PICKUP_STREET_ADDRESS" SIZE=8  MAXLENGTH=30 value="<?php echo $pickupStreetAddress;?>">
	<span class="error" style="color:red">* <?php echo $rentalStreetAddressErr;?></span></TD></TR>
  </TR>
  <TR>
	<TD width="35%">&nbsp;</TD>
	<td width="40%" valign=middle>&nbsp;</td>
   ...
   <TR>
	<TD width="35%">&nbsp;</TD>
	<td width="40%" valign=middle>&nbsp;</td>
	<TD width="25%"><INPUT TYPE=text NAME="PICKUP_CITY_NAME" SIZE=8  MAXLENGTH=30 value="<?php echo $pickupCity;?>">
	<span class="error" style="color:red">* <?php echo $rentalCityErr;?></span></TD></TR>
  </TR>
  ...
  <TR>
	<TD width="35%">&nbsp;</TD>
	<td width="40%" valign=middle>&nbsp;</td>
	<TD width="25%"><SELECT NAME="PICKUP_COUNTRY_CODE">
		<OPTION VALUE="AL">Albania</OPTION><OPTION VALUE="AD">Andorra</OPTION><OPTION 
...
(Federation of)</OPTION><OPTION VALUE="ZB">Zambia</OPTION><OPTION VALUE="KF">Zimbabwe</OPTION></SELECT>
</TD></TR>
  </TR>
<TR>
...
<fieldset><legend><h3>Return Location Information(only needed if different from pickup location)</h3></legend>
	<span class="error" style="color:red"> <?php echo $returnLocationInformationErr;?></span>
<TABLE WIDTH="100%" CELLSPACING="0" CELLPADDING="0" BORDER="0">
   <TR>
	<TD width="35%">&nbsp;</TD>
	<td width="40%" valign=middle><h2>-OR-</h2></td>
 ...
 <TR>
   ...
   <TR>
	<TD width="35%"><INPUT TYPE=text NAME="RETURN_LOCATION_CODE" SIZE=3  MAXLENGTH=3 value="<?php echo $returnLocationCode;?>">(if known) </TD>
	<td width="40%" valign=middle>&nbsp;</td>
	<TD width="25%"><INPUT TYPE=text NAME="RETURN_STREET_ADDRESS" SIZE=8  MAXLENGTH=30 value="<?php echo $returnStreetAddress;?>">
		<span class="error" style="color:red">* <?php echo $returnStreetAddressErr;?></span></TD></TR>
  ...
  <TR>
   ...
   <TR>
	<TD width="35%">&nbsp;</TD>
	<td width="40%" valign=middle>&nbsp;</td>
	<TD width="25%"><INPUT TYPE=text NAME="RETURN_CITY_NAME" SIZE=8  MAXLENGTH=30 value="<?php echo $returnCity;?>">
	<span class="error" style="color:red">* <?php echo $returnCityErr;?></span></TD></TR>
	</TD></TR>
  </TR>
  ...
  <TR>
	<TD width="35%">&nbsp;</TD>
	<td width="40%" valign=middle>&nbsp;</td>
	<TD width="25%"><SELECT NAME="RETURN_STATE"  width=165 value="<?php echo $returnState;?>"><OPTION VALUE="0">Please Select One</OPTION><OPTION VALUE="Not Applicable">Not Applicable</OPTION><OPTION VALUE="Alabama">Alabama</OPTION><OPTION VALUE="Alaska">Alaska</OPTION><OPTION VALUE="Arizona">Arizona</OPTION><OPTION VALUE="Arkansas">Arkansas</OPTION><OPTION VALUE="California">California</OPTION><OPTION VALUE="Colorado">Colorado</OPTION><OPTION VALUE="Connecticut">Connecticut</OPTION><OPTION VALUE="Delaware">Delaware</OPTION><OPTION VALUE="District of Columbia">District of Columbia</OPTION><OPTION VALUE="Florida">Florida</OPTION><OPTION VALUE="Georgia">Georgia</OPTION><OPTION VALUE="Hawaii">Hawaii</OPTION><OPTION VALUE="Idaho">Idaho</OPTION><OPTION VALUE="Illinois">Illinois</OPTION><OPTION VALUE="Indiana">Indiana</OPTION><OPTION VALUE="Iowa">Iowa</OPTION><OPTION VALUE="Kansas">Kansas</OPTION><OPTION VALUE="Kentucky">Kentucky</OPTION><OPTION VALUE="Louisiana">Louisiana</OPTION><OPTION VALUE="Maine">Maine</OPTION><OPTION VALUE="Maryland">Maryland</OPTION><OPTION VALUE="Massachusetts">Massachusetts</OPTION><OPTION VALUE="Michigan">Michigan</OPTION><OPTION VALUE="Minnesota">Minnesota</OPTION><OPTION VALUE="Mississippi">Mississippi</OPTION><OPTION VALUE="Missouri">Missouri</OPTION><OPTION VALUE="Montana">Montana</OPTION><OPTION VALUE="Nebraska">Nebraska</OPTION><OPTION VALUE="Nevada">Nevada</OPTION><OPTION VALUE="New Hampshire">New Hampshire</OPTION><OPTION VALUE="New Jersey">New Jersey</OPTION><OPTION VALUE="New Mexico">New Mexico</OPTION><OPTION VALUE="New York">New York</OPTION><OPTION VALUE="North Carolina">North Carolina</OPTION><OPTION VALUE="North Dakota">North Dakota</OPTION><OPTION VALUE="Ohio">Ohio</OPTION><OPTION VALUE="Oklahoma">Oklahoma</OPTION><OPTION VALUE="Oregon">Oregon</OPTION><OPTION VALUE="Pennsylvania">Pennsylvania</OPTION><OPTION VALUE="Rhode Island">Rhode Island</OPTION><OPTION VALUE="South Carolina">South Carolina</OPTION><OPTION VALUE="South Dakota">South Dakota</OPTION><OPTION VALUE="Tennessee">Tennessee</OPTION><OPTION VALUE="Texas">Texas</OPTION><OPTION VALUE="Utah">Utah</OPTION><OPTION VALUE="Vermont">Vermont</OPTION><OPTION VALUE="Virginia">Virginia</OPTION><OPTION VALUE="Washington">Washington</OPTION><OPTION VALUE="West Virginia">West Virginia</OPTION><OPTION VALUE="Wisconsin">Wisconsin</OPTION><OPTION VALUE="Wyoming">Wyoming</OPTION</SELECT>
		<span class="error" style="color:red">* <?php echo $returnStateErr;?></span></TD></TR></TD></TR>
  </TR>
  <TR>
	<TD width="35%">&nbsp;</TD>
	<td width="40%" valign=middle>&nbsp;</td>
...
<fieldset><legend><h3>Car Selection</h3></legend>
	<span class="error" style="color:red"> <?php echo $carClassErr;?></span>
...
<TABLE WIDTH="100%" CELLSPACING="0" CELLPADDING="0" BORDER="0">
	...
	<tr>
		<td>First Name: <INPUT TYPE=text NAME="FIRST_NAME" SIZE=20  MAXLENGTH=12 value="<?php echo $firstName;?>">
		<span class="error" style="color:red">* <?php echo $firstNameErr;?></span></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	</tr>	
	<tr>
		<td>Last Name: <INPUT TYPE=text NAME="LAST_NAME" SIZE=20  MAXLENGTH=12 value="<?php echo $lastName;?>">
		<span class="error" style="color:red">* <?php echo $lastNameErr;?></span></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	</tr>	
	<tr>
		<td>Email Address: <INPUT TYPE=text NAME="EMAIL_ADDRESS" SIZE=20  MAXLENGTH=12 value="<?php echo $email;?>">
